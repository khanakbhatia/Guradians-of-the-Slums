...
const evacuationRoutes = asyncHandler(async (req, res) => {
  const result = await beeAIService.planEvacuation(req.body);
  res.status(200).json(new ApiResponse(200, result, 'Evacuation routes generated'));
});

module.exports = { analyze, riskScore, generateReport, assignVolunteers, explainRisk, evacuationRoutes };
